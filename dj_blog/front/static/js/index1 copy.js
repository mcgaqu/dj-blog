...
function selectLanguage(e) {
    ...
    const params = {
        'i18n': true,
        'filtro': `&active=1&internal=0&alias__startswith=asinex`,
        'idElement': 'Mc_layout_grade',
    }     
...
async function loadPage(page, language) {
    const filtro = `?format=json&note=${page}&grade=${language}`;
...
async function loadSubMenu(item) {
    ...
    filtro = `format=json&active=1&${filtro}`;
...
async function manageLayout(model, params) {
    ...
    if (params.i18n) {filtro = `${filtro}&grade=${localStorage.language}`};
    ...
    const defaultMarks = [
        ...
        // 'loadLanguages', 'selectLanguage'
        // 'loadSubMenu', 'selectSubMenu', 
        // 'loadDashboard', 
    ]
    ...
    data.forEach(function(item) {
        ...
        console.log(`--- model=${model} alias=${item.alias} mark=${mark}`)
        switch (mark) {
            case 'loadLanguages':
                loadLanguages(item);
                break;
