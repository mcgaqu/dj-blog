...
class ModelTree(ModelBase):
    ...
    # root_alias = models.CharField(max_length=150, null=True, blank=True)
    

    class Meta(ModelBase.Meta):
        abstract = True

    def ME_num_children(self):
        return self.children.count()
    ME_num_children.short_description = 'Children'
    ...
    # Raiz = property(_getRoot)       

    def _get_pos_sort(self):
        # return "%s__%s" % (self.grade, self.sort)
        sort = "" if not self.sort else self.sort
        if not self.parent:
            return sort
        else:
            return "%s_%s" % (self.parent.pos, sort)
 

    def save(self, *args, **kwargs):
        # import pdb; pdb.set_trace()
        self.level = self._get_level()
        self.pos = self._get_pos_sort()
        return super().save(*args, **kwargs)


class ModelTree1(ModelTree):    
    root_alias = models.CharField(max_length=150, null=True, blank=True)
    last_alias = models.CharField(max_length=150, null=True, blank=True)
    ...
    def _get_alias(self):
        if not self.parent:
            return "%s" % self.last_alias            
        else:
            return "%s__%s" % (self.parent.alias, self.last_alias)


    def save(self, *args, **kwargs):
        self.alias = self._get_alias()
        self.root_alias = self._getRoot().last_alias
        return super().save(*args, **kwargs)

#======================================

"""
class ModelTreeX(ModeloBase):
