...
class ModelBase(models.Model):
    ...
    # MC_date = property(MC_datet)

    def MC_datet(self):
        if self.datet:
            return self.datet.strftime('%d/%m/%Y %H:%M:%S')
        else:
            return ''
    MC_datet.admin_order_field = 'datet'
    MC_datet.short_description = 'datet'
    # MC_datet = property(MC_datet)
    ...
    def get_company_obj(self):
        if hasattr(self, 'company'):
            return self.company
        else:
            return get_company()

    def get_biz_obj(self):
        if hasattr(self, 'biz'):
            return self.biz
        else:
            return get_biz()
