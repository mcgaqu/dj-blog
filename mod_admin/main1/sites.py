...
class AdminSite1(admin.AdminSite):
    ...
    site_header = "Administración de: %s" % site_name.upper()
            ...
            try:
                ...
                try:
                    ...
                    if settings.SITE_NAME != site.name:
                        if True: # not settings.BIZ_ID:
                            site.name = self.site_name
                        else:
                            site.name = "%s__%s" % (self.site_name, self.site_name)
                        site.domain = '%s' % settings.SITE_DOMAIN
                        site.save()
